...
export class PropagationTimeoutError extends Error {
  constructor(subdomain: string, timeoutMs: number) {
    super(`Domain "${subdomain}" was stored but did not become routable within ${timeoutMs}ms`);
...
export async function waitForPropagation(
  ...
  throw new PropagationTimeoutError(subdomain, timeoutMs);
}
...
domainsRouter.post('/api/domains/register', verifyApiKey, async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      if (err instanceof PropagationTimeoutError) {
        console.error(`[domains] ${err.message}`);
        res.status(502).json({ error: 'Domain was stored but is not routable yet. Please retry registration.' });
        return;
      }
      throw err;
    }
...
domainsRouter.put('/api/domains/update', verifyApiKey, async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      if (err instanceof PropagationTimeoutError) {
        console.error(`[domains] ${err.message}`);
        res.status(502).json({ error: 'Domain was updated but is not routable yet. Please retry deployment.' });
        return;
      }
      throw err;
    }
