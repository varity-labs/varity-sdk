...
function sendUnscopedRootAssetMiss(appName: string, res: Response): boolean {
  if (!isRootAssetSegment(appName)) return false;

  res.status(404)
    .type('text/plain')
    .send('Asset path is missing an app prefix');
  return true;
}
...
proxyRouterV2.all('/:appName', async (req: Request, res: Response, next) => {
  ...
  if (sendUnscopedRootAssetMiss(appName, res)) {
    return;
  }
