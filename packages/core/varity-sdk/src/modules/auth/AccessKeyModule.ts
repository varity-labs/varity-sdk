...
export class AccessKeyModule {
   ...
   * ```
   */
  async createAccessKey(options: CreateAccessKeyOptions): Promise<AccessKey> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to create access key: ${response.statusText}. ${
          errorData.message || ''
        }`
      )
    }
    ...
    return {
      ...
      status: data.status as AccessKeyStatus,
  ...
  async listAccessKeys(): Promise<AccessKey[]> {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(
        `Failed to list access keys: ${response.statusText}. ${
          errorData.message || ''
        }`
      )
    }
    ...
    return data.accessKeys.map((key: any) => ({
      ...
      status: key.status as AccessKeyStatus,
   ...
   * ```
   */
  async getAccessKey(accessKeyId: string): Promise<AccessKey> {
    const apiEndpoint = this.sdk.getAPIEndpoint()
    const apiKey = this.sdk.getAPIKey()
    ...
    const response = await fetch(
      `${apiEndpoint}/api/v1/auth/access-keys/${accessKeyId}`,
      {
        headers: {
          'Authorization': `Bearer ${apiKey}`
        }
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(
        `Failed to get access key: ${response.statusText}. ${
          errorData.message || ''
        }`
      )
    }
   ...
   * ```
   */
  async revokeAccessKey(accessKeyId: string): Promise<void> {
    const apiEndpoint = this.sdk.getAPIEndpoint()
    const apiKey = this.sdk.getAPIKey()
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(
        `Failed to revoke access key: ${response.statusText}. ${
          errorData.message || ''
        }`
      )
    }
  ...
  async updateAccessKey(
    accessKeyId: string,
    updates: UpdateAccessKeyOptions
  ): Promise<AccessKey> {
    const apiEndpoint = this.sdk.getAPIEndpoint()
    const apiKey = this.sdk.getAPIKey()
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(
        `Failed to update access key: ${response.statusText}. ${
          errorData.message || ''
        }`
      )
    }
    ...
    return [
      {
        resource: 'bucket:*',
        actions: [
          'storage:GetObject' as Action,
          'storage:PutObject' as Action,
          'storage:DeleteObject' as Action,
          'storage:ListObjects' as Action
        ],
