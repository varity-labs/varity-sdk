...
export class MonitoringModule {
   ...
   * ```
   */
  async queryMetrics(options: QueryMetricsOptions): Promise<MetricSeries> {
    const apiEndpoint = this.sdk.getAPIEndpoint()
    const apiKey = this.sdk.getAPIKey()
   ...
   * ```
   */
  async startTrace(name: string, tags?: Record<string, string>): Promise<TraceContext> {
    const apiEndpoint = this.sdk.getAPIEndpoint()
    const apiKey = this.sdk.getAPIKey()
  ...
  async getErrors(options: {
    ...
    const response = await fetch(
      `${apiEndpoint}/api/v1/monitoring/errors?${queryParams}`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
   ...
   * ```
   */
  async recordEvent(name: string, data?: Record<string, any>): Promise<void> {
    const apiEndpoint = this.sdk.getAPIEndpoint()
    const apiKey = this.sdk.getAPIKey()
...
export class TraceContext {
   ...
   */
  async startSpan(name: string, tags?: Record<string, string>): Promise<SpanContext> {
