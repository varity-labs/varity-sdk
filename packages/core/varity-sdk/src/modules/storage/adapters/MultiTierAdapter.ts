...
export class MultiTierAdapter extends BaseStorageAdapter {
  ...
  private hotTierAdapter: FilecoinAdapter
  private coldTierAdapter: FilecoinAdapter
  private config: MultiTierStorageConfig
  private tieringMetadata: Map<string, TieringMetadata>
  private autoTieringInterval: NodeJS.Timeout | null = null

  constructor(sdk: VaritySDK, config: MultiTierStorageConfig) {
    super(StorageBackend.MULTI_TIER as any)
    this.sdk = sdk
    this.config = config
  ...
  async download(identifier: string): Promise<Buffer> {
    ...
    if (metadata) {
      ...
      if (
        this.config.autoTiering.enabled &&
   ...
   */
  async promoteTier(identifier: string, targetTier: StorageTier): Promise<void> {
    const metadata = await this.loadMetadata(identifier)
    if (!metadata || metadata.tier === targetTier) return
   ...
   * @param targetTier - Target tier
   */
  async demoteTier(identifier: string, targetTier: StorageTier): Promise<void> {
   ...
   */
  private determineInitialTier(data: Buffer | string | Blob, options: UploadOptions): StorageTier {
    let size = 0
   ...
   */
  private getAdapterForTier(tier: StorageTier): FilecoinAdapter {
   ...
   */
  private shouldPromote(metadata: TieringMetadata): boolean {
   ...
   */
  private shouldDemote(metadata: TieringMetadata): boolean {
    const daysSinceAccess =
      (Date.now() - metadata.lastAccessed.getTime()) / (24 * 60 * 60 * 1000)
   ...
   * @param targetTier - Target tier
   */
  private async schedulePromotion(identifier: string, targetTier: StorageTier): Promise<void> {
   ...
   */
  private async runAutoTiering(): Promise<void> {
   ...
   * @param identifier - Storage identifier
   */
  private async persistMetadata(identifier: string): Promise<void> {
   ...
   */
  private async loadMetadata(identifier: string): Promise<TieringMetadata | null> {
   ...
   * @param identifier - Storage identifier
   */
  private async deleteMetadata(identifier: string): Promise<void> {
    const metadataKey = `metadata:${identifier}`
    try {
