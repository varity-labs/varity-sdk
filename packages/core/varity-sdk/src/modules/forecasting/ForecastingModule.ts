...
export class ForecastingModule {
   ...
   * ```
   */
  async predict(options: PredictOptions): Promise<ForecastResult> {
   ...
   * ```
   */
  async analyzeTrends(options: TrendAnalysisOptions): Promise<TrendAnalysisResult> {
   ...
   * ```
   */
  async simulateScenario(options: ScenarioSimulationOptions): Promise<ScenarioResult> {
   ...
   * ```
   */
  async trainModel(options: TrainModelOptions): Promise<TrainedModel> {
   ...
   * ```
   */
  async evaluateModel(options: EvaluateModelOptions): Promise<EvaluationResult> {
  ...
  async listModels(filters?: {
    ...
    const response = await fetch(
      `${this.sdk.getAPIEndpoint()}/api/v1/forecasting/models?${params.toString()}`,
      {
        headers: {
          ...(this.sdk.getAPIKey() && { 'X-API-Key': this.sdk.getAPIKey() })
        }
