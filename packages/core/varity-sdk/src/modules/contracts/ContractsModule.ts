...
export class ContractsModule {
  ...
  private static readonly ABI_REGISTRY: Record<string, any> = {
   ...
   */
  private getContractInstance(contractName: string, withSigner: boolean = false): ethers.Contract {
    ...
    if (!abi) {
      throw new Error(
        `Contract ABI not found for "${contractName}". ` +
        `Available contracts: ${Object.keys(ContractsModule.ABI_REGISTRY).join(', ')}`
      )
    }

    const address = this.sdk.getContractAddress(contractName as any)
    const provider = this.sdk.getProvider()

    const contract = new ethers.Contract(
      address,
      abi,
      withSigner ? this.sdk.getSigner() : provider
    )
   ...
   */
  async call(contractName: string, method: string, args: any[] = []): Promise<any> {
  ...
  async getContractAddress(contractName: string): Promise<string> {
    return this.sdk.getContractAddress(contractName as any)
  }
   ...
   * @param eventName - Event name
   */
  async stopListening(contractName: string, eventName: string): Promise<void> {
    const listenerId = `${contractName}:${eventName}`
  ...
  async batchCall(calls: ContractCall[]): Promise<any[]> {
    const promises = calls.map(call =>
      this.call(call.contractName, call.method, call.args)
    )
   ...
   * @returns Array of results
   */
  async multicall(contractName: string, calls: MultiCall[]): Promise<any[]> {
    const contract = this.getContractInstance(contractName, false)
