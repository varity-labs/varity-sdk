...
class DeploymentOrchestrator:
    ...
    def deploy(
        ...
        try:
            ...
            # Step 2.8: Inject <base> tag for subdirectory hosting
            if subdomain and hosting != "akash":
                from .path_rewriter import inject_base_tag
                base_modified = inject_base_tag(build_artifacts.output_dir, f"/{subdomain}/")
                if base_modified:
                    self._log(f"   Injected base path for /{subdomain}/ in {base_modified} files")
            ...
            if hosting == "akash":
                # Deploy to Akash Network via AkashDeployService
                ...
                frontend_url = akash_deploy_result.provider_url
                cid = f"akash:{akash_deploy_result.deployment_id}"
            ...
            if hosting == "akash" and akash_deploy_result and akash_deploy_result.success:
                try:
                    ...
                    register_akash_domain(
                        subdomain=subdomain,
                        deployment_url=akash_deploy_result.provider_url,
                        deployment_id=akash_deploy_result.deployment_id,
            ...
            result = DeploymentResult(
                ...
                provider_url=akash_deploy_result.provider_url if akash_deploy_result else None,
                estimated_monthly_cost=akash_deploy_result.estimated_monthly_cost if akash_deploy_result else 0.0,
                free_credits_remaining=akash_deploy_result.free_credits_remaining if akash_deploy_result else 0.0,
                docker_image=akash_deploy_result.docker_image if akash_deploy_result else None,
            )
    ...
    def _create_manifest(
        ...
        if hosting == "akash" and akash_result:
            # akash_result can be AkashDeployResult (new service) or AkashDeploymentResult (legacy)
            if hasattr(akash_result, 'provider_url'):
                # New AkashDeployResult from AkashDeployService
                manifest["akash"] = {
                    "deployment_id": akash_result.deployment_id,
                    "provider_url": akash_result.provider_url,
                    "docker_image": akash_result.docker_image,
                    "estimated_monthly_cost": akash_result.estimated_monthly_cost,
                    "free_credits_remaining": akash_result.free_credits_remaining,
                }
            else:
                # Legacy AkashDeploymentResult from AkashConsoleDeployer
                manifest["akash"] = {
                    "deployment_id": akash_result.deployment_id,
    ...
    def _deploy_to_akash_service(
        ...
        """
        Deploy to compute platform using AkashDeployService.

        Handles the full pipeline: build Docker image, push to GHCR,
        deploy via Akash Console API, return clean URLs.
        ...
        Returns:
            AkashDeployResult with URLs and cost info
        ...
        try:
            ...
            # Execute deployment
            service = AkashDeployService(verbose=self.verbose)
            result = service.deploy(config)

            if not result.success:
                raise DeploymentError(result.error_message)
            ...
            raise DeploymentError(f"Compute deployment failed: {e}")
    ...
    def _deploy_to_akash(
        ...
        """
        Deploy to Akash Network using Console API (legacy path).

        Used when AkashDeployService is not available.
