...
class IPFSUploader:
    ...
    def __init__(self, client_id: Optional[str] = None):
        ...
        # Find the upload script — check bundled package location first,
        # then fall back to the development repo location
        bundled_path = Path(__file__).parent.parent / "scripts" / "upload_to_ipfs.js"
        repo_path = Path(__file__).parent.parent.parent / "scripts" / "upload_to_ipfs.js"

        if bundled_path.exists():
            self.script_path = bundled_path
        elif repo_path.exists():
            self.script_path = repo_path
        else:
            raise FileNotFoundError(
                f"IPFS upload script not found.\n"
                f"Checked:\n"
                f"  {bundled_path}\n"
                f"  {repo_path}\n"
