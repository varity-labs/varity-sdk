...
from varitykit.core.config import ConfigManager, Environment
...
def init(ctx, project_name, template, path, yes, force, github):
    ...
    Examples:
        ...
        varitykit init --template saas-starter --path ./projects/my-app
        varitykit init  (interactive mode)
        varitykit init --github  (add GitHub Actions deployment workflow)
    """
    console = Console()
    ...
    try:
        ...
        console.print(
            Panel.fit(
                ...
                f"  2. npm install\n"
                f"  3. npm run dev\n\n"
                f"[cyan]When you're ready to deploy:[/cyan]\n"
                f"  varitykit app deploy\n\n"
                f"[cyan]Resources:[/cyan]\n"
                f"  Docs:    [link=https://docs.varity.so]https://docs.varity.so[/link]\n"
                f"  Discord: [link=https://discord.gg/7vWsdwa2Bg]https://discord.gg/7vWsdwa2Bg[/link]\n"
                f"  Help:    varitykit --help",
                border_style="green",
            )
        )
